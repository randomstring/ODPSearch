...
KEYWORD::KEYWORD (PIDBOBJ DbParent): DOCTYPE (DbParent) {
  SetNumBuckets(38); 
}


GPTYPE 
KEYWORD::ParseWords(unsigned char* DataBuffer, // Pointer to document text buffer.
...
float 
KEYWORD::ScoreHit(IRESULT result, float termweight) {
...
void 
KEYWORD::GetMdtData(unsigned char* DataBuffer, 
		     INT4 DataLength, 
		     GPTYPE DataOffset,
		     long pathName,
		     long fileName,
